...
class MySQLCodec extends Codec
{
    ...
    public function encodeCharacter($immune, $c)
    {
    	...
    	// Check for alphanumeric characters
  		...
  		switch($this->mode)
  		{
  			case self::MYSQL_ANSI: return $encodedOutput.$this->encodeCharacterANSI($c);
  			case self::MYSQL_STD: return $encodedOutput.$this->encodeCharacterMySQL($c);  
  		}  		
    ...
    public function decodeCharacter($input)
    {
    	...
    	if(mb_substr($_4ByteEncodedInput,0,1,"UTF-32") === null)
    	{
    		...
    		return array('decodedCharacter'=>null,'encodedString'=>null);
   	  	...
   	  	switch($this->mode)
  		{
  			case self::MYSQL_ANSI: return $this->decodeCharacterANSI($_4ByteEncodedInput);
  			case self::MYSQL_STD: return $this->decodeCharacterMySQL($_4ByteEncodedInput);  
  		}
	 ...
	 *  
	 * @param input
	 * 			A string containing characters you'd like decoded
	 * @return
    ...
    private function decodeCharacterMySQL($input)
    {
    	...
    	switch($ordinalValue)
    	{
    		case self::ORD_VALUE_0:$second = $this->normalizeEncoding( chr(0x00) ); break;
    		case self::ORD_VALUE_B:$second = $this->normalizeEncoding( chr(0x08) ); break;
    		case self::ORD_VALUE_T:$second = $this->normalizeEncoding( chr(0x09) ); break;
    		case self::ORD_VALUE_N:$second = $this->normalizeEncoding( chr(0x0a) ); break;
    		case self::ORD_VALUE_R:$second = $this->normalizeEncoding( chr(0x0d) ); break;
    		case self::ORD_VALUE_Z:$second = $this->normalizeEncoding( chr(0x1a) ); break;
    		case self::ORD_VALUE_DQUOTE:$second = $this->normalizeEncoding( chr(0x22) ); break;
    		case self::ORD_VALUE_PERCENT:$second = $this->normalizeEncoding( chr(0x25) ); break;
    		case self::ORD_VALUE_QUOTE:$second = $this->normalizeEncoding( chr(0x27) ); break;
    		case self::ORD_VALUE_BSLASH:$second = $this->normalizeEncoding( chr(0x5c) ); break;
    		case self::ORD_VALUE_UNDERSCORE:$second = $this->normalizeEncoding( chr(0x5f) ); break;
    	}
