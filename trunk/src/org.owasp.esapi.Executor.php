...
require_once("errors/org.owasp.esapi.ExecutorException.php");
...
class Executor implements IExecutor {
    ...
    public function Executor() {
    	 $this->logger = $ESAPI->getLogger("ESAPI", "Executor");
    }
    ...
    public function executeSystemCommand($executable, $params, $workdir, $timeoutSeconds) {
        $br = null;
        try {
            $this->logger->logTrace(Logger::SECURITY, "Initiating executable: " + $executable + " " + $params + " in " + $workdir);
            ...
            $i = new ArrayObject($params).getIterator();
            while ($i.valid()) {
                $param = $i->current();
            ...
            $command = (String[])params.toArray( new String[0] );
