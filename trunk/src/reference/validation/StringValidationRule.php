...
/**
 * StringValidationRule implementation of the ValidationRule interface.
 *
 * PHP version 5.2.9
 *
 * @category  OWASP
...
class StringValidationRule extends BaseValidationRule
{
    ...
    public function __construct($typeName, $encoder = null, $whiteListPattern = null)
    {
        ...
        else if ($whiteListPattern !== null)
        {
            throw new InvalidArgumentException(
                'Validation misconfiguration - constructor expected a string $whiteListPattern'
            );
        }
    }
     ...
     *
     * @param  $pattern non-empty string whitelist regex pattern.
     */
    public function addWhitelistPattern($pattern)
    {
        if (! is_string($pattern)) {
            throw new InvalidArgumentException(
                'Validation misconfiguration - addWhitelistPattern expected a string $pattern'
            );
        }
        if ($pattern == '') {
     ...
     * Inputs will be validated against each pattern.
     *
     * @param  $pattern non-empty string blacklist regex pattern.
     */
    public function addBlacklistPattern($pattern)
    {
        if (! is_string($pattern)) {
            throw new InvalidArgumentException(
                'Validation misconfiguration - addBlacklistPattern expected string $pattern'
            );
        }
        if ($pattern == '') {
            ESAPI::getLogger()->warning(
     ...
     *
     * @param  $length minimum length of the canonicalized Input below which
    ...
    public function setMinimumLength($length)
    {
        if (! is_numeric($length)) {
            throw new InvalidArgumentException(
                'Validation misconfiguration - setMinimumLength expected numeric $length'
            );
        }
     ...
     * the input is deemed invalid.
     *
     * @param  $length maximum length of the canonicalized Input above which
     *         it is deemed invalid.
     */
    public function setMaximumLength($length)
    {
        if (! is_numeric($length)) {
            throw new InvalidArgumentException(
                'Validation misconfiguration - setMaximumLength expected numeric $length'
            );
        }
    ...
    public function getValid($context, $input)
    {
        ...
        if ($this->minLength > $this->maxLength) {
            throw new RuntimeException(
                'Validation misconfiguration - $minLength should not be greater than $maxLength!'
            );
        }
        ...
        if ($length < $this->minLength) {
            throw new ValidationException(
                $context . ': Invalid input. Input was shorter than the Minimum length of ' . $this->minLength . ' characters.',
                'Length of Input was less than the minimum length of ' . $this->minLength,
                $context
            );
        }
        if ($length > $this->maxLength) {
            throw new ValidationException(
                $context . ': Invalid input. Input was longer than the Maximum length of ' . $this->maxLength . ' characters.',
                'Length of Input was more than the maximum length of ' . $this->maxLength,
                $context
            );
        }
        ...
        foreach($this->whitelistPatterns as $pattern)
        {
            if (! preg_match("/{$pattern}/", $canonical)){
                throw new ValidationException(
                    $context . ': Invalid input. Please conform to the regex ' . $pattern,
                    $context . ': Invalid input. Input does not conform to the whitelist regex ' . $pattern,
                    $context
                );
            }
        }
        ...
        foreach($this->blacklistPatterns as $pattern)
        {
            if (preg_match("/{$pattern}/", $canonical)) {
                throw new ValidationException(
                    $context . ': Invalid input. Dangerous input matching ' . $pattern,
                    $context . ': Invalid input. Input matches the blacklist regex ' . $pattern,
                    $context
                );
            }
        }
